  ...
  [
    {
      ...
      after: { entity_type: "character", name: "Alice", summary_md: "주인공.", attributes: { age: 18 } },
    },
  ],
  null,
  2,
...
function safeJsonParse(text: string): { ok: true; value: unknown } | { ok: false; error: string } {
  const raw = (text || "").trim();
  if (!raw) return { ok: false, error: "JSON 데이터를 입력해주세요." };
  try {
...
function humanStatus(status: string): string {
  if (status === "proposed") return "제안됨 (아직 적용되지 않음)";
...
export function MemoryUpdateDrawer(props: Props) {
  ...
  const runPropose = useCallback(async () => {
    if (!chapterId) {
      toast.toastError("먼저 장을 선택해 주세요.");
      return;
    }
    ...
    try {
      ...
      if (!ops.length)
        throw new ApiError({
          code: "INVALID_OPS",
          message: "JSON은 `ops` 배열이거나 `ops` 필드를 포함하는 객체여야 합니다.",
          requestId: "local",
          status: 0,
        });
      ...
      toast.toastSuccess("제안서가 작성되었습니다.");
    } catch (e) {
      const err =
  ...
  const runAutoPropose = useCallback(async () => {
    ...
    try {
      ...
      setProposeResult(res.data);
      toast.toastSuccess("제안서가 생성되었습니다 (자동).");
    } catch (e) {
      const err =
        e instanceof ApiError
          ? e
  ...
  const runApplyAccepted = useCallback(async () => {
    ...
    try {
      ...
      if (!changeSetId) {
        throw new ApiError({
          code: "BAD_RESPONSE",
          message: "`change_set.id`가 누락되었습니다.",
  ...
  const retryApply = useCallback(async () => {
    if (!lastApplyChangeSetId) {
      toast.toastError("다시 시도할 수 있는 `change_set_id`가 없습니다.");
      return;
    }
    setApplyLoading(true);
    setApplyError(null);
  ...
  const refreshStructured = useCallback(async () => {
    if (!projectId) {
      toast.toastError("프로젝트 ID가 누락되었습니다.");
      return;
    }
