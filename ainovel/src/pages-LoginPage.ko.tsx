      ...
      <div className="mx-auto flex min-h-screen max-w-screen-sm items-center px-4 py-12">
        <div className="w-full">
                ...
                <div className="rounded-atelier border border-border bg-canvas p-3">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div className="text-xs text-subtext">
                      ...
                      <div className="mt-1">로그인 없이 먼저 체험해볼 수 있어요. 권한·협업·멀티유저가 필요하면 그때 다시 로그인하세요.</div>
                    </div>
                    <button
                      ...
                      type="button"
                    >
                      로그인 건너뛰기, {UI_COPY.auth.continueInDevFallback}
                    </button>
                  </div>
                </div>
                <DebugDetails title="자세한 설명 (선택)">
            ...
            <form className="mt-6 grid gap-6" onSubmit={(event) => void handleSubmit(event)}>
              ...
              <div>
                ...
                <div className="mb-2 text-center text-xs text-subtext">
                  {UI_COPY.auth.noAccountHint} 또는 아래 방식으로 바로 선택
                </div>
          ...
          <div className="mt-4 text-center text-xs text-subtext">
            ...
            <div className="mt-1">비밀번호를 잊으셨나요? 현재 버전에서는 관리자에게 초기화를 요청해주세요 (MVP 단계 — 자가 복구는 아직 지원하지 않습니다).</div>
          </div>
        </div>
      </div>
    </div>
