...
function mergeManagedAdvancedExtra(form: LlmForm, extraBase: Record<string, unknown>): BuildPayloadResult {
  ...
  if (provider === "anthropic") {
    if (form.anthropic_thinking_enabled) {
      ...
      if (budget === null) {
        return { ok: false, message: "Anthropic의 토큰 예산(budget_tokens)은 양의 정수여야 합니다." };
      }
  ...
  if (provider === "gemini") {
    ...
    if (form.gemini_thinking_budget.trim() && budget === null) {
      return { ok: false, message: "제미니, 예산은 양의 정수여야 합니다." };
    }
...
export function buildPresetPayload(form: LlmForm): BuildPayloadResult {
  ...
  if (!model) return { ok: false, message: "모델은 비어 있을 수 없습니다." };
  ...
  if (!isRecord(parsed)) return { ok: false, message: "extra는 유효한 JSON 객체여야 합니다." };
